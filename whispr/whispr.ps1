...
Start-Transcript $ENV:TEMP\whispr.log -Append
...
if (!([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    Write-Output "whispr needs to be run as Administrator. Attempting to relaunch."
    Start-Process -Verb runas -FilePath powershell.exe -ArgumentList "iwr -useb https://hangthe.dev/whispr | iex"
    break
}
Function Get-WhisprCheckBoxes {
    ...
    <#
        ...
        Get-WhisprCheckBoxes "WPFInstall"
    ...
    if($Group -eq "WPFInstall"){
        $filter = Get-WhisprVariables -Type Checkbox | Where-Object {$psitem -like "WPFInstall*"}
    ...
    if($Group -eq "WPFTweaks"){
        $filter = Get-WhisprVariables -Type Checkbox | Where-Object {$psitem -like "WPF*Tweaks*"}
        $CheckBoxes = $sync.GetEnumerator() | Where-Object {$psitem.Key -in $filter}
        Foreach ($CheckBox in $CheckBoxes){
            if($CheckBox.value.ischecked -eq $true){
    ...
    if($Group -eq "WPFFeature"){
        $filter = Get-WhisprVariables -Type Checkbox | Where-Object {$psitem -like "WPF*Feature*"}
        $CheckBoxes = $sync.GetEnumerator() | Where-Object {$psitem.Key -in $filter}
        Foreach ($CheckBox in $CheckBoxes){
            if($CheckBox.value.ischecked -eq $true){
                $Output.Add($Checkbox.Name)
...
function Get-WhisprInstallerProcess {
    <#
...
function Get-WhisprRegistry {
    <#
        ...
        Set-WhisprRegistry -Name "PublishUserActivities" -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\System" -Type "DWord" -Value "0"
...
Function Get-WhisprToggleStatus {
    <#
...
function Install-WhisprChoco {
    ...
    try{
        ...
        if((Test-WhisprPackageManager -choco)){
...
Function Install-WhisprProgramWinget {
...
function Install-WhisprWinget {
    ...
    Try{
        ...
        if (Test-WhisprPackageManager -winget) {
        ...
        Start-Process -Verb runas -FilePath powershell.exe -ArgumentList "irm https://raw.githubusercontent.com/ChrisTitusTech/Whispr/main/winget.ps1 | iex"
...
function Invoke-WhisprBingSearch {
        <#
...
Function Invoke-WhisprCurrentSystem {
...
Function Invoke-WhisprDarkMode {
        <#
...
function Invoke-WhisprFeatureInstall {
    <#
...
function Invoke-WhisprScript {
    <#
        ...
        Invoke-WhisprScript -ScriptBlock $scriptblock -Name "Hello World"
...
function Invoke-WhisprTweaks {
    ...
    if($sync.configs.tweaks.$CheckBox.ScheduledTask){
        $sync.configs.tweaks.$CheckBox.ScheduledTask | ForEach-Object {
            Set-WhisprScheduledTask -Name $psitem.Name -State $psitem.$($values.ScheduledTask)
        }
    }
    if($sync.configs.tweaks.$CheckBox.service){
        $sync.configs.tweaks.$CheckBox.service | ForEach-Object {
            Set-WhisprService -Name $psitem.Name -StartupType $psitem.$($values.Service)
        }
    }
    if($sync.configs.tweaks.$CheckBox.registry){
        $sync.configs.tweaks.$CheckBox.registry | ForEach-Object {
            Set-WhisprRegistry -Name $psitem.Name -Path $psitem.Path -Type $psitem.Type -Value $psitem.$($values.registry)
        }
    }
    if($sync.configs.tweaks.$CheckBox.$($values.ScriptType)){
        $sync.configs.tweaks.$CheckBox.$($values.ScriptType) | ForEach-Object {
            $Scriptblock = [scriptblock]::Create($psitem)
            Invoke-WhisprScript -ScriptBlock $scriptblock -Name $CheckBox
        }
    }
    ...
    if(!$undo){
        if($sync.configs.tweaks.$CheckBox.appx){
            $sync.configs.tweaks.$CheckBox.appx | ForEach-Object {
                Remove-WhisprAPPX -Name $psitem
            }
        }

    }
}
function Remove-WhisprAPPX {
    <#
        ...
        Remove-WhisprAPPX -Name "Microsoft.Microsoft3DViewer"
...
function Set-WhisprDNS {
    <#
        ...
        Set-WhisprDNS -DNSProvider "google"
...
function Set-WhisprRestorePoint {
    <#
    ...
    if ($existingRestorePoints.Count -eq 0) {
        $description = "System Restore Point created by Whispr"
        
        Checkpoint-Computer -Description $description -RestorePointType "MODIFY_SETTINGS"
    }
}
function Set-WhisprScheduledTask {
    <#
        ...
        Set-WhisprScheduledTask -Name "Microsoft\Windows\Application Experience\Microsoft Compatibility Appraiser" -State "Disabled"
...
Function Set-WhisprService {
    <#
        ...
        Set-WhisprService -Name "HomeGroupListener" -StartupType "Manual"
...
function Set-WhisprUITheme {
    <#
        ...
        Set-WhisprUITheme -inputXAML $inputXAML
...
function Test-WhisprPackageManager {
    <#
...
Function Update-WhisprProgramWinget {
...
function Invoke-WPFButton {
    ...
    Switch -Wildcard ($Button){
        ...
        "WPFToggleDarkMode" {Invoke-WPFDarkMode -DarkMoveEnabled $(Get-WhisprDarkMode)}
        ...
        "WPFWhisprShortcut" {Invoke-WPFShortcut -ShortcutToAdd "Whispr"}
...
function Invoke-WPFFeatureInstall {
    ...
    if($sync.ProcessRunning){
        ...
        [System.Windows.MessageBox]::Show($msg, "whispr", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Warning)
        return
    }

    $Features = Get-WhisprCheckBoxes -Group "WPFFeature"
    ...
    Invoke-WPFRunspace -ArgumentList $Features -ScriptBlock {
        ...
        Invoke-WhisprFeatureInstall $Features
...
Function Invoke-WPFFormVariables {
    ...
    Write-Host " __      _| |__ (_)___ _ __  _ __ ___  (_)_ __  ___| |_ __ _| | | ___ _ __  "
    Write-Host " \ \ /\ / / '_ \| / __| '_ \| '__/ __| | | '_ \/ __| __/ _` | | |/ _ \ '__| "
    Write-Host "  \ V  V /| | | | \__ \ |_) | |  \__ \ | | | | \__ \ || (_| | | |  __/ |    "
    Write-Host "   \_/\_/ |_| |_|_|___/ .__/|_|  |___/ |_|_| |_|___/\__\__,_|_|_|\___|_|    "
    Write-Host "                      |_|                                                   "
    Write-Host ""
    Write-Host ""
...
function Invoke-WPFGetInstalled {
    ...
    if(!(Test-WhisprPackageManager -winget) -and $checkbox -eq "winget"){
        Write-Host "==========================================="
    ...
    Invoke-WPFRunspace -ArgumentList $checkbox -ScriptBlock {
        ...
        $Checkboxes = Invoke-WhisprCurrentSystem -CheckBox $checkbox
...
function Invoke-WPFImpex {
    ...
    if ($type -eq "export"){
        $jsonFile = Get-WhisprCheckBoxes $checkbox -unCheck $false
...
function Invoke-WPFInstall {
    ...
    $WingetInstall = Get-WhisprCheckBoxes -Group "WPFInstall"
    ...
    Invoke-WPFRunspace -ArgumentList $WingetInstall -scriptblock {
        ...
        try{
            ...
            Install-WhisprWinget

            # Install all winget programs in new window
            Install-WhisprProgramWinget -ProgramsToInstall $WingetInstall
...
function Invoke-WPFPresets {
    ...
    if($checkbox -eq "WPFTweaks"){
        $filter = Get-WhisprVariables -Type Checkbox | Where-Object {$psitem -like "*tweaks*"}
        $sync.GetEnumerator() | Where-Object {$psitem.Key -in $filter} | ForEach-Object {
...
function Invoke-WPFTab {
    ...
    Param ($ClickedTab)
    $Tabs = Get-WhisprVariables | Where-Object {$psitem -like "WPFTab?BT"}
    $TabNav = Get-WhisprVariables | Where-Object {$psitem -like "WPFTabNav"}
...
function Invoke-WPFToggle {
    ...
    Switch -Wildcard ($Button){
        ...
        "WPFToggleDarkMode" {Invoke-WhisprDarkMode -DarkMoveEnabled $(Get-WhisprToggleStatus WPFToggleDarkMode)}
        "WPFToggleBingSearch" {Invoke-WhisprBingSearch $(Get-WhisprToggleStatus WPFToggleBingSearch)}
...
function Invoke-WPFtweaksbutton {
  ...
  $Tweaks = Get-WhisprCheckBoxes -Group "WPFTweaks"

  Set-WhisprDNS -DNSProvider $sync["WPFchangedns"].text
...
function Invoke-WPFundoall {
    ...
    Invoke-WPFRunspace -ArgumentList $Tweaks -ScriptBlock {
        ...
        Foreach ($tweak in $tweaks){
            Invoke-WhisprTweaks $tweak -undo $true
        }
...
function Invoke-WPFUnInstall {
    ...
    Invoke-WPFRunspace -ArgumentList $WingetInstall -scriptblock {
        ...
        try{
            ...
            # Install all winget programs in new window
            Install-WhisprProgramWinget -ProgramsToInstall $WingetInstall -Manage "Uninstalling"
...
$inputXML = '<Window x:Class="Whisprity.MainWindow"
        xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
        xmlns:d="http://schemas.microsoft.com/expression/blend/2008"
        xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006"
        xmlns:local="clr-namespace:Whisprity"
        mc:Ignorable="d"
                    ...
                    <TabItem Header="Tweaks" Visibility="Collapsed" Name="WPFTab2">
                        <Grid Background="#333333">
							...
							<Label Content="Shortcuts" />
                                <Button Name="WPFWhisprShortcut" Content="Create Whispr Shortcut" HorizontalAlignment = "Left" Margin="5,0" Padding="20,5" Width="300"/>
...
$sync.configs.tweaks = '{
  ...
  "WPFEssTweaksOO": {
    "InvokeScript": [
      "curl.exe -s \"https://raw.githubusercontent.com/ChrisTitusTech/Whispr/main/ooshutup10_Whispr_settings.cfg\" -o $ENV:temp\\ooshutup10.cfg
  ...
  "WPFEssTweaksRemoveEdge": {
    "InvokeScript": [
        ...
        curl.exe -s \"https://raw.githubusercontent.com/ChrisTitusTech/Whispr/main/edgeremoval.bat\" -o $ENV:temp\\edgeremoval.bat
...
#Add functions
$functions = Get-ChildItem function:\ | Where-Object {$_.name -like "*Whispr*" -or $_.name -like "*WPF*"}
...
if ((Get-WhisprToggleStatus WPFToggleDarkMode) -eq $True){
    $ctttheme = 'Matrix'
}
Else{
    $ctttheme = 'Classic'
...
$inputXML = Set-WhisprUITheme -inputXML $inputXML -themeName $ctttheme
...
$sync.keys | ForEach-Object {
    if($sync.$psitem){
        if(
            ...
            $sync["$psitem"].IsChecked = Get-WhisprToggleStatus $sync["$psitem"].Name
